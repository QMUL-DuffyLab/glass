...
begin	
	using Graphs, GraphPlot, StatsBase, Colors, CairoMakie
	include("src/Simulations.jl")
	import .Simulations
	pp = Simulations.PulseParams(200e-12, 50e-12, 485e-9, 5e12)
	sim = Simulations.SimulationParams(15e-9, 1e-12, 1e6, 1e-9, 25e-12, pp, 10000, 5)
	#(bins, counts, labels, ec) = Simulations.generate_histogram(sim, lattice)
	pulse = Simulations.construct_pulse(pp, sim.dt1)
	times = 0:sim.dt1:2*pp.μ
	f = Figure()
	ax = Axis(f[1, 1], limits=(first(times), last(times), 0, maximum(pulse)))
	lines!(ax, times[1:length(pulse)], pulse)
	f
end
...
begin
	...
	import .Lattices
	lattice = Lattices.lattice_generation(Lattices.get_lattice("hex"), 200, [Lattices.get_protein("lh2"), Lattices.get_protein("lhcii")], [0.5, 0.5])
	Lattices.plot_lattice(lattice)
end
